...
```terraform
# RSA signing key set (most common for JWT signing)
...
# ECDSA signing key set (smaller keys, faster operations)
...
- `algorithm` (String) The algorithm for the key. Valid values: `RS256`, `ES256`, `ES512`, `HS256`, `HS512`. **Cannot be changed after creation.**
- `key_id` (String) The Key ID (`kid`) for the generated key. **Cannot be changed after creation.**
- `set_id` (String) The ID of the JSON Web Key Set. **Cannot be changed after creation.**
- `use` (String) The intended use. Valid values: `sig` (signature) or `enc` (encryption). **Cannot be changed after creation.**
